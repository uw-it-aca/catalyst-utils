...
from catalyst_utils.dao.file import read_file, write_file, build_archive
...
class FileDAOFunctionsTest(TestCase):
    ...
    def test_build_archive(self):
        archive = build_archive([self.test_path])
        self.assertIsInstance(archive, BytesIO)
        self.assertRaises(ObjectDoesNotExist, build_archive, [])
